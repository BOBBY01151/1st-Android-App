...
function getVehicleTextPosition(vehicle) {
    ...
    try {
        ...
        // Calculate world position attached to vehicle's left side
        const textWorldPos = {
...
function updateBrowserPosition(worldPos) {
    ...
    try {
        // Convert 3D world position to screen coordinates (this happens every frame)
        ...
        if (!screenPos || (screenPos.x === 0 && screenPos.y === 0)) {
            // Position conversion failed, hide browser
            browser.active = false;
            return;
        }
        
        // Validate screen position (check if behind camera or out of bounds)
        if (screenPos.x < -0.5 || screenPos.x > 1.5 || screenPos.y < -0.5 || screenPos.y > 1.5) {
            // Text is outside view, hide it
            browser.active = false;
            return;
        }
        
        // Apply light smoothing to reduce jitter (minimal, just for stability)
        ...
        smoothedScale = smoothedScale + (targetScale - smoothedScale) * 0.2; // Light smoothing for scale
        
        // Update browser with current position
        browser.active = true;
...
mp.events.add('render', () => {
    ...
    // Convert 3D position to screen coordinates EVERY FRAME
    // This makes the text appear "attached" to the vehicle
