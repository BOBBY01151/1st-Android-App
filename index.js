...
const positionSmoothing = 0.15; // Light smoothing to reduce jitter while still following vehicle
...
function updateBrowserPosition(worldPos) {
    ...
    try {
        ...
        // Check if position is behind camera or too far off screen
        if (screenPos.x < -0.2 || screenPos.x > 1.2 || screenPos.y < -0.2 || screenPos.y > 1.2) {
            browser.active = false;
            return;
        }
        
        // Apply light smoothing to reduce jitter while still following vehicle
        smoothedScreenX = smoothedScreenX + (screenPos.x - smoothedScreenX) * positionSmoothing;
        smoothedScreenY = smoothedScreenY + (screenPos.y - smoothedScreenY) * positionSmoothing;
        ...
        smoothedScale = smoothedScale + (targetScale - smoothedScale) * 0.2;
        
        // Update browser with smoothed position
        browser.active = true;
...
function startVehicleDisplay(vehicle, seat) {
    ...
    // Initialize smoothed values with current position
    const vehiclePos = vehicle.getPosition();
    try {
        const camPos = mp.game.cam.getGameplayCamCoord();
        const distance = Math.sqrt(
            Math.pow(vehiclePos.x - camPos.x, 2) +
            Math.pow(vehiclePos.y - camPos.y, 2) +
            Math.pow(vehiclePos.z - camPos.z, 2)
        );
        smoothedScale = Math.max(0.7, Math.min(1.3, 12 / Math.max(distance, 5)));
    } catch (e) {
        smoothedScale = 1.0;
    }
...
mp.events.add('render', () => {
    ...
    // Update browser position - converts 3D to screen coordinates every frame
