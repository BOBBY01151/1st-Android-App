...
function updateBrowserPosition(worldPos) {
    ...
    try {
        ...
        // If conversion fails or position is off-screen, hide the browser
        if (!screenPos || screenPos.x < -0.2 || screenPos.x > 1.2 || screenPos.y < -0.2 || screenPos.y > 1.2) {
            browser.active = false;
            return;
        }
        ...
        // Calculate scale based on distance (smooth scale changes slightly)
        const targetScale = Math.max(0.7, Math.min(1.3, 12 / Math.max(distance, 5)));
        smoothedScale = smoothedScale + (targetScale - smoothedScale) * 0.2; // Light smoothing for scale only
        
        // Update browser with current screen position - NO SMOOTHING on position
        // This keeps the text truly attached to the vehicle
        browser.active = true;
...
mp.events.add('render', () => {
    ...
    // This keeps the text naturally attached to the vehicle's 3D position
