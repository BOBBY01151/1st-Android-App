...
let positionSmoothing = 0.85; // Smoothing for position updates (reduced from 0.95 for better responsiveness)
let scaleSmoothing = 0.3; // Smoothing for scale updates
...
function updateBrowserPosition(worldPos) {
    ...
    try {
        ...
        // This will update every frame based on current camera position
        ...
        if (!screenPos || (screenPos.x === 0 && screenPos.y === 0)) {
            // If conversion fails, hide the browser
            browser.active = false;
            return;
        }
        
        // Validate screen position - if out of bounds, hide
        if (screenPos.x < -0.5 || screenPos.x > 1.5 || screenPos.y < -0.5 || screenPos.y > 1.5) {
            browser.active = false;
            return;
        }
        
        // Smooth the screen position to reduce jitter
        // This will always update based on camera movement
        ...
        // Update browser with smoothed position (always updates with camera)
        browser.active = true;
...
function startVehicleDisplay(vehicle, seat) {
    ...
    // Reset smoothed values when starting new display
    smoothedScreenX = 0.5;
    smoothedScreenY = 0.5;
    smoothedScale = 1.0;
...
mp.events.add('render', () => {
    ...
    // Update browser position - converts 3D to screen with smoothing
    // This will update every frame based on current camera position
